#!/usr/bin/env python3
"""
davina_recovery_dashboard.py - local read-only recovery transcript dashboard.
...
state so Fifi/the internal team can inspect recovery status without adding a new
...
WATCHDOG_STATE_DIR = HOME / ".openclaw/davina-missed-reply-watchdog"
...
try:
    import davina_missed_reply_watchdog as watchdog
...
def queue_summary(records: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for record in records:
        if record.get("recoverable") is True:
            recoverable += 1
        if record.get("agentId") == "davina-helowedding":
            davina += 1
...
<head>
  ...
  <title>Davina Recovery Dashboard</title>
  <style>
...
<body>
  <h1>Davina Recovery Dashboard</h1>
...
class DashboardHandler(BaseHTTPRequestHandler):
    server_version = "DavinaRecoveryDashboard/1.0"
    ...
    def is_authorized(self, query: dict[str, list[str]]) -> bool:
        token = os.environ.get("DAVINA_RECOVERY_DASHBOARD_TOKEN", "").strip()
        if not token:
            return True
...
def main() -> int:
    ...
    print(f"{iso_now()} Davina recovery dashboard listening on http://{args.host}:{args.port}")
    if not os.environ.get("DAVINA_RECOVERY_DASHBOARD_TOKEN", "").strip():
        print("Token auth disabled. Keep this bound to 127.0.0.1 or set DAVINA_RECOVERY_DASHBOARD_TOKEN.")
    try:
