#!/usr/bin/env python3
"""
davina_group_context.py - local group-context summarizer for Davina HeloWedding.

The script reads Davina's existing OpenClaw WhatsApp group session files, strips
...
SESSIONS_INDEX = Path(
    os.environ.get(
        "DAVINA_GROUP_CONTEXT_SESSIONS_INDEX",
        HOME / ".openclaw/agents/davina-helowedding/sessions/sessions.json",
    )
)
STATE_DIR = Path(os.environ.get("DAVINA_GROUP_CONTEXT_STATE_DIR", HOME / ".openclaw/davina-group-context"))
DB_PATH = Path(os.environ.get("DAVINA_GROUP_CONTEXT_DB", STATE_DIR / "group-context.sqlite"))
INTERNAL_TEAM_CONTACTS = HOME / ".openclaw/workspace-davina-helowedding/knowledge/INTERNAL_TEAM_CONTACTS.md"
ESCALATION_CONTACTS = HOME / ".openclaw/workspace-davina-helowedding/knowledge/ESCALATION_CONTACTS.md"

TZ = ZoneInfo("Asia/Jakarta")
AGENT_ID = "davina-helowedding"
...
def authorize(requester_phone: str) -> dict[str, Any]:
    ...
    if phone not in contacts:
        raise ValueError("requester_phone is not a verified Helo internal contact")
...
def clean_event(event: dict[str, Any]) -> CleanMessage | None:
    ...
    return CleanMessage(
        ...
        sender_id=sender_id,
        sender_name=sender_name.strip() or ("Sender" if role == "user" else "Davina"),
        text=text,
    )
...
def summarize_messages(group: dict[str, Any], messages: list[CleanMessage], include_snippets: bool) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "counts": {
            ...
            "davina_messages": len(assistant_messages),
...
def summarize_group(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not groups:
        raise ValueError("matching Davina group session was not found")
