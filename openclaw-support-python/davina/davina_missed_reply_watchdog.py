#!/usr/bin/env python3
"""
davina_missed_reply_watchdog.py - LLM-composed missed-reply watchdog for Davina.

Default mode is dry-run. Live mode sends exactly one context-aware reply per
eligible unreplied Davina WhatsApp chat and mirrors it into the local session.
"""
...
SESSIONS_INDEX = HOME / ".openclaw/agents/davina-helowedding/sessions/sessions.json"
OPENCLAW = HOME / ".npm-global/bin/openclaw"
DEEPSEEK_KEY_PATH = HOME / ".openclaw/credentials/davina-helowedding/deepseek-api-key.txt"
INTERNAL_TEAM_CONTACTS = HOME / ".openclaw/workspace-davina-helowedding/knowledge/INTERNAL_TEAM_CONTACTS.md"
INTERCEPTED_QUEUE = HOME / ".openclaw/security/guard-intercepted-inbound.jsonl"
STATE_DIR = HOME / ".openclaw/davina-missed-reply-watchdog"
DB_PATH = STATE_DIR / "watchdog.sqlite"
LOG_PATH = STATE_DIR / "watchdog.log"

CHANNEL = "whatsapp"
ACCOUNT_ID = "davina-helowedding"
FIFI_PHONE = "+628118871666"
...
def group_is_eligible(session_key: str, user_text: str, metadata: dict[str, Any]) -> bool:
    ...
    return "davina" in lowered or "@davina" in lowered
...
def context_for(events: list[dict[str, Any]]) -> str:
    ...
    for event in reversed(selected):
        role = role_of(event)
        text = strip_metadata(content_text(event))
        label = "Sender" if role == "user" else "Davina"
...
def load_candidates(min_age_ms: int, max_age_ms: int, current_ms: int) -> list[ChatCandidate]:
    ...
    for session_key, data in index.items():
        if not isinstance(data, dict):
            continue
        if not session_key.startswith("agent:davina-helowedding:whatsapp:"):
            continue
        ...
        if not sender_name and category == "fifi":
            sender_name = "Fifi"
        candidates.append(
            ChatCandidate(
...
def load_intercepted_candidates(
    ...
    for line in lines[-2000:]:
        ...
        if not isinstance(record, dict):
            continue
        if record.get("agentId") != "davina-helowedding" or record.get("recoverable") is not True:
            continue
        ...
        if not session_key:
            session_key = f"agent:davina-helowedding:whatsapp:direct:{target}"
        candidates.append(
            ChatCandidate(
                session_key=session_key,
                session_file=None,
...
def compose_reply(candidate: ChatCandidate, timeout: int) -> str:
    ...
    system_prompt = (
        "You are Davina, Helo Wedding's WhatsApp customer-service assistant and delayed missed-reply recovery responder. "
        ...
        "while staying professional, safe, and within Davina's Helo Wedding role. "
        ...
        "If information is missing, ask the smallest useful clarification or say the Helo team needs to confirm. "
        "For sender category fifi, respond as a concise business assistant to Helo's boss. "
        "For sender category internal_team, respond as internal Helo team coordination, not as a wedding lead; do not ask for calon pengantin intake. "
        "For internal team, use the sender's preferred name exactly as supplied when known and never use generic Mas/Mbak. "
        "For sender category external_contact, respond as normal Davina customer/vendor support. "
        "If the latest inbound is only thanks, acknowledgement, arrival status, or does not need Davina to reply, "
    ...
    user_prompt = (
        ...
        f"Recent chat context (last up to {MAX_CONTEXT_USER_TURNS} sender turns plus visible Davina replies):\n"
        f"{candidate.context}\n\n"
        "Write Davina's next reply now."
    )
    payload = {
...
def append_transcript_mirror(candidate: ChatCandidate, reply_text: str, message_id: str) -> bool:
    ...
    try:
        ...
        entry = {
            "type": "message",
            "id": f"davina-watchdog-{int(current_dt.timestamp() * 1000)}",
            ...
            "message": {
                ...
                "source": "davina-missed-reply-watchdog",
                "delivery": {
                    "channel": CHANNEL,
