#!/usr/bin/env python3
"""
davina_calendar_reminders.py - send due Davina calendar reminders to approved targets.

The internal calendar service owns event/reminder creation and due marking. This
dispatcher only consumes due Davina reminders, sends them through Davina's
WhatsApp account to Fifi or approved Helo Wedding group targets, then updates
reminder status for auditability.
"""
...
STATE_DIR = HOME / ".openclaw/davina-calendar-reminders"
LOG_FILE = STATE_DIR / "davina-calendar-reminders.log"
OPENCLAW = HOME / ".npm-global/bin/openclaw"

AGENT_ID = "davina-helowedding"
CHANNEL = "whatsapp"
ACCOUNT_ID = "davina-helowedding"
FIFI_TARGET = "+628118871666"
HELO_WO_CREW_GROUP_TARGET = "628111082248-1621650280@g.us"
ALLOWED_TARGETS = {
    FIFI_TARGET,
    HELO_WO_CREW_GROUP_TARGET,
}
...
def dispatch(conn: sqlite3.Connection, dry_run: bool, limit: int) -> dict[str, Any]:
    ...
    for reminder in reminders:
        ...
        if target not in ALLOWED_TARGETS:
            failed += 1
            mark_failed(conn, reminder, "Davina calendar reminders may only target approved Helo recipients")
