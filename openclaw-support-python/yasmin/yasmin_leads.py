#!/usr/bin/env python3
"""
yasmin_leads.py - non-sensitive local lead ledger for Yasmin ZahiraWedding.
...
WORKSPACE = HOME / ".openclaw/workspace-yasmin-zahirawedding"
INTERNAL_TEAM_CONTACTS = WORKSPACE / "knowledge/INTERNAL_TEAM_CONTACTS.md"
STATE_DIR = Path(os.environ.get("YASMIN_LEADS_STATE_DIR", str(HOME / ".openclaw/yasmin-leads")))
...
def validate_requester(phone: Any) -> str:
    ...
    if not requester:
        raise PermissionError("requester is not an authorized Zahira Wedding internal team member")
    return requester
...
def main() -> int:
    ...
    for name, help_text in (
        ("record-json", "Record/update one non-sensitive Yasmin lead"),
        ("list-json", "List Yasmin leads for verified Zahira internal requesters"),
        ("get-json", "Get one Yasmin lead for verified Zahira internal requesters"),
